...
namespace GameEngine
{
	public static class Resources
	{
		...
		private static void AutoloadResources()
		{
			...
			//Save assets which could be imported with an AssetManager into a dictionary,with AssetManagers being the keys.
			var allGameFiles = Directory.GetFiles("Assets","*.*",SearchOption.AllDirectories);
		...
		public static T ImportFromStream<T>(Stream stream,AssetManager<T> assetManager = null,string fileName = null) where T : class
		{
			...
			Utils.ObjectOrCollectionCall<Asset>(output,asset => asset.InitAsset(),false);
			return output;
		}
		
		public static string ImportText(string filePath,bool addToCache = false,bool throwOnFail = true)
		{
			return Import(filePath,addToCache,(AssetManager<string>)assetManagers[".txt"][0],throwOnFail);
		}
		public static byte[] ImportBytes(string filePath,bool addToCache = false,bool throwOnFail = true)
		{
			return Import(filePath,addToCache,(AssetManager<byte[]>)assetManagers[".bytes"][0],throwOnFail);
		}
		#endregion
		#region Get
		...
		public static T Get<T>(string filePath) where T : class
		{
			...
			ReadyPath(ref filePath);
			NameToPath(ref filePath,out bool ntpMultiplePaths);
			if(cacheByPath.TryGetValue(filePath,out var obj)) {
				if(obj is T content) {
					return content;
				}
			}
		...
		public static T Find<T>(string assetName) where T : class
		{
			var type = typeof(T);
