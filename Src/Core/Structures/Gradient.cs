...
namespace Dissonance.Engine
{
	public class Gradient<T>
	{
		...
		static Gradient()
		{
			Gradient<float>.LerpFunc = Mathf.Lerp;
			Gradient<double>.LerpFunc = (left, right, t) => Mathf.Lerp(left, right, t);

			Gradient<sbyte>.LerpFunc = (left, right, t) => (sbyte)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<byte>.LerpFunc = (left, right, t) => (byte)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<short>.LerpFunc = (left, right, t) => (short)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<ushort>.LerpFunc = (left, right, t) => (ushort)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<int>.LerpFunc = (left, right, t) => (int)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<uint>.LerpFunc = (left, right, t) => (uint)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<long>.LerpFunc = (left, right, t) => (long)Math.Round(Mathf.Lerp(left, right, t));
			Gradient<ulong>.LerpFunc = (left, right, t) => (ulong)Math.Round(Mathf.Lerp(left, right, t));
