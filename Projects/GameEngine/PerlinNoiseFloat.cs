...
namespace GameEngine
{
	public class PerlinNoiseFloat
	{
		...
		public float GetValue(float x,float y = 0f,float z = 0f)
		{
			float result = 0f;
			float pers = 1f;
			//int seedCurrent;
			...
			return ((float)result+1f)/2f;
		}
		internal static float CoherentNoise(float x,float y,float z,int seed,QualityMode quality)
		{
			...
			float n0 = Noise(x,y,z,x0,y0,z0,seed);
			float n1 = Noise(x,y,z,x1,y0,z0,seed);
			...
			ix0 = Mathf.Lerp(n0,n1,xs);
			n0 = Noise(x,y,z,x0,y1,z1,seed);
			n1 = Noise(x,y,z,x1,y1,z1,seed);
			ix1 = Mathf.Lerp(n0,n1,xs);
			float iy1 = Mathf.Lerp(ix0,ix1,ys);
			return Mathf.Lerp(iy0,iy1,zs);
		}
		internal static float Noise(float fx,float fy,float fz,int ix,int iy,int iz,int seed)
		{
		...
		internal static float MapCubicSCurve(float value) => value*value*(3f-2f*value);
		...
		internal static float MakeInt32Range(float value)
		{
			if(value>=1073741824.0) {
				return 2f*((float)Math.IEEERemainder(value,1073741824.0))-1073741824f;
			}
			if(value<=-1073741824.0) {
				return 2f*((float)Math.IEEERemainder(value,1073741824.0))+1073741824f;
			}
